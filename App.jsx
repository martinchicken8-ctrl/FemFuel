  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // the recipe library reset (see nicinsync_recipes_v6_own) runs
        ...
        if (key === "nicinsync_recipes" && window.localStorage.getItem("nicinsync_recipes_v6_own")) return;
...
export default function App() {
  ...
  useEffect(() => {
    try {
      if (!window.localStorage.getItem("nicinsync_recipes_v6_own")) {
        setRecipes(buildConvertedOpenRecipes());
        window.localStorage.setItem("nicinsync_recipes_v6_own", "1");
      }
    } catch (err) {
