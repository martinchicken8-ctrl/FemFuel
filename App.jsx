...
import { CYCLE_RECIPE_DB, OPEN_RECIPE_DB } from "./recipeData.js";
import {
  Home,
  Salad,
  BookOpen,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Guard against a stale encrypted blob overwriting a just-completed hard reset:
        // the recipe library reset (see nicinsync_recipes_v7_cuisines) runs
        // synchronously on mount, but this decrypt is async and can resolve afterwards
        // with old cached data - skip restoring it in that case.
        if (key === "nicinsync_recipes" && window.localStorage.getItem("nicinsync_recipes_v7_cuisines")) return;
...
// ---------------------------------------------------------------------------
const MEAT_KEYS_OR = ["rind", "hähnchen", "huhn", "hühnchen", "pute", "schwein", "lamm", "speck", "wurst", "schinken", "hack", "hackfleisch", "kalb", "hase", "ente", "gans"];
const FISH_KEYS_OR = ["garnele", "lachs", "thunfisch", "fisch", "forelle", "kabeljau", "krabbe", "hummer", "scholle", "hering", "seelachs"];
const DAIRY_KEYS_OR = ["käse", "butter", "sahne", "milch", "joghurt", "parmesan", "quark", "schmand", "frischkäse"];
const EGG_KEYS_OR = ["ei", "eier"];
const GLUTEN_KEYS_OR = ["mehl", "brot", "nudel", "nudeln", "pasta", "brötchen", "toast", "paniermehl", "gebäck", "baguette"];

function convertOpenRecipeToAppRecipe(r, idx) {
  const phase = PHASES[r.phase];
  const allIngText = r.ingredients.join(" ").toLowerCase();
  const hasAny = (keys) => keys.some((k) => hasWholeWordMatch(allIngText, k));
  ...
  return {
    ...
    steps: r.steps,
    why: `${phase.focus.join(", ")} \u2013 passend f\u00fcr die ${phase.name}.`,
    color: phase.color,
    meat,
    fish,
    dairy,
...
// Whole-word matching for SHORT food keywords: a naive lower.includes("ei") would also
// match inside "ein", "kein", "mein", "weiß" etc., so short keys require the match to be
// bounded by non-letters. Longer keys skip this and use plain substring matching instead,
// because German freely builds compound words without spaces (e.g. "käse" legitimately
// needs to match inside "Frischkäse", "Ziegenkäse") - enforcing boundaries there would
// cause false negatives instead of fixing false positives.
// Scales the leading amount in an ingredient line (e.g. "250 g Mehl" -> "375 g Mehl" at
// 1.5x) for the recipe modal's serving-count stepper. Leaves the line untouched if it
// doesn't start with a recognizable number (e.g. "Salz und Pfeffer").
function scaleIngredientAmount(line, factor) {
  if (factor === 1) return line;
  const fractionMatch = line.match(/^(\d+)\s*\/\s*(\d+)(.*)$/);
  if (fractionMatch) {
    const value = (parseInt(fractionMatch[1], 10) / parseInt(fractionMatch[2], 10)) * factor;
    return `${formatScaledNumber(value)}${fractionMatch[3]}`;
  }
  const numMatch = line.match(/^(\d+(?:[.,]\d+)?)(.*)$/);
  if (numMatch) {
    const value = parseFloat(numMatch[1].replace(",", ".")) * factor;
    return `${formatScaledNumber(value)}${numMatch[2]}`;
  }
  return line;
}
...
function hasWholeWordMatch(lowerText, key) {
  if (key.length > 3) return lowerText.includes(key);
...
const INITIAL_RECIPES = buildConvertedOpenRecipes();
...
export default function App() {
  ...
  // One-time hard reset: the recipe library now contains ONLY the open-source recipe
  // collection (previously it also included a custom cycle-recipe database, which has
  // been removed). This wipes whatever was seeded before and replaces it cleanly,
  // without touching the user's own favorites list (favoriteIds) or shortcuts.
  useEffect(() => {
    try {
      if (!window.localStorage.getItem("nicinsync_recipes_v7_cuisines")) {
        setRecipes(buildConvertedOpenRecipes());
        window.localStorage.setItem("nicinsync_recipes_v7_cuisines", "1");
      }
    } catch (err) {
      console.error("Recipe reset failed:", err);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);

  // Shared English translation sync: our recipe library is authored in German. When the
  // app's language is set to English, this instantly applies whatever translations the
  // server has already produced (GET, fast and free - benefits every visitor equally),
  // and if the shared library isn't fully translated yet, this browser also contributes
  // one small batch toward finishing it (POST). Progress is stored centrally in Vercel
  // Blob, so it converges to "done for everyone" after a handful of total visits rather
  // than repeating per browser.
  useEffect(() => {
    if (lang !== "en") return;
    let cancelled = false;
    ...
    function applyTranslations(translations) {
      ...
      setRecipes((prev) =>
        prev.map((r) => {
          ...
          return tr ? { ...r, title: tr.title, ingredients: tr.ingredients, steps: tr.steps } : r;
        })
      );
    }
    ...
    (async () => {
      try {
        const getRes = await fetch("/api/translations");
        if (!getRes.ok || cancelled) return;
        const { translations, count } = await getRes.json();
        applyTranslations(translations);

        const allOpenRecipes = buildConvertedOpenRecipes();
        if (cancelled || count >= allOpenRecipes.length) return;

        const postRes = await fetch("/api/translations", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ recipes: allOpenRecipes }),
        });
        if (!postRes.ok || cancelled) return;
        const postResult = await postRes.json();
        applyTranslations(postResult.translations);
      } catch (err) {
        console.warn("Shared English translation sync failed (will retry next load).", err);
      }
    })();
  ...
  // Companion to the sync above: switching back to German restores the canonical German
  // text for our base recipe library (translations only ever change the display copy,
  // never the underlying German source) - without touching recipes the user generated
  // via AI/pantry mode, which aren't part of this base library.
  useEffect(() => {
    if (lang !== "de") return;
    const germanBase = new Map(buildConvertedOpenRecipes().map((r) => [r.id, r]));
    setRecipes((prev) =>
      prev.map((r) => {
        const base = germanBase.get(r.id);
        return base ? { ...r, title: base.title, ingredients: base.ingredients, steps: base.steps } : r;
      })
    );
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [lang]);
...
function HomeTab({ phase, phaseKey, cycleDay, todayKey, setCycleStartDate, onOpenProfile, goToRecipe, recipes, diets, hormoneScore, userName, rewards }) {
  ...
  const dailyRecipes = useMemo(() => {
    const pool = recipes.filter((r) => r.phase === phaseKey);
    const source = pool.length >= 3 ? pool : recipes;
    const picks = [];
    const usedIds = new Set();
    for (let i = 0; i < 3 && source.length > 0; i++) {
      const pick = seededPick(
        source.filter((r) => !usedIds.has(r.id)),
        `${phaseKey}-${cycleDay}-${i}`
      );
      if (pick) {
        picks.push(pick);
...
function RecipeDetailModal({ recipe, onClose, onAddToShoppingList, isFavorite, onToggleFavorite, onSaveAsShortcut }) {
  ...
  const scaledIngredients = useMemo(() => recipe.ingredients.map((ing) => scaleIngredientAmount(ing, scaleFactor)), [recipe.ingredients, scaleFactor]);
      ...
      >
        ...
        <div className="p-5">
          ...
          <div className="flex items-center justify-between mb-2 flex-wrap gap-2">
            <p className="text-xs font-semibold flex items-center gap-1.5" style={{ color: "#121212" }}>
              <ListChecks size={14} style={{ color: rPhase.color }} /> {t("ingredients_label")}
            </p>
            <button
          ...
          <div className="flex items-center gap-2 rounded-full px-1 py-1 mb-3 w-fit" style={{ background: "#F5F5F5" }}>
              <button
              ...
              <span className="text-xs font-semibold w-16 text-center" style={{ color: "#121212" }}>
                {servings} {servings === 1 ? t("serving_singular") : t("serving_plural")}
              </span>
              <button
          ...
          <ul className="mb-5">
            {scaledIngredients.map((ing, i) => (
              <li key={i} className="text-xs py-1.5 flex items-center gap-2" style={{ color: "#262626", borderBottom: i < scaledIngredients.length - 1 ? "1px solid #EFEFEF" : "none" }}>
