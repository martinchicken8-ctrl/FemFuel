...
If isJunk is true, "swap" must be {"name":"...","kcal":0,"protein":0,"carbs":0,"fat":0,"match":0,"why":"..."}, otherwise "swap" must be null.`;
  ...
  if (!response.ok) {
    ...
    try {
      ...
      detail = errBody && errBody.error ? `: ${JSON.stringify(errBody.error).slice(0, 200)}` : `: ${JSON.stringify(errBody).slice(0, 200)}`;
    } catch (e) {
      /* body wasn't JSON */
    }
      ...
      <div className="px-5 pt-8">
        ...
        {lastAiError && (
          <div className="rounded-2xl p-4 mb-4" style={{ background: "#F6E1E3" }}>
            ...
            <p className="text-[11px] font-mono break-words" style={{ color: "#8C3B47" }}>
              {lastAiError}
            </p>
          </div>
        )}
