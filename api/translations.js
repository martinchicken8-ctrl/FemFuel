    ...
    if (req.method === "POST") {
      ...
      for (let i = 0; i < batch.length; i++) {
        ...
        try {
          ...
          if (err.message && err.message.includes("429")) rateLimited = true;
        }
        if (i < batch.length - 1 && !rateLimited) await new Promise((r) => setTimeout(r, BATCH_DELAY_MS));
      }

      if (translatedThisRun > 0) {
        try {
          await writeTranslations(translations);
        } catch (err) {
          res.status(500).json({ error: `Blob write failed: ${err.message}` });
          return;
        }
      }
