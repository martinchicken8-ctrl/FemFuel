  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // the recipe library reset (see nicinsync_recipes_v4_open_only_force) runs
        ...
        if (key === "nicinsync_recipes" && window.localStorage.getItem("nicinsync_recipes_v4_open_only_force")) return;
...
export default function App() {
  ...
  useEffect(() => {
    try {
      if (!window.localStorage.getItem("nicinsync_recipes_v4_open_only_force")) {
        setRecipes(buildConvertedOpenRecipes());
        window.localStorage.setItem("nicinsync_recipes_v4_open_only_force", "1");
      }
    } catch (err) {
  ...
  // Shared translation sync: on load, instantly apply whatever German translations the
  // server has already produced (GET, fast and free - benefits every visitor equally,
  // not just the first one). If the shared library isn't fully translated yet, this
  // browser also contributes one small batch toward finishing it (POST) - since progress
  // is stored centrally in Vercel Blob, it converges to "done for everyone" after a
  // handful of total visits rather than repeating per browser.
  useEffect(() => {
    if (lang !== "de") return;
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        // Contribute one small batch in the background (low priority, doesn't block UI)
        ...
        console.warn("Shared translation sync failed (will retry next load).", err);
      }
    })();
